...
const configurePassport = ()=> {
    ...
    async (accessToken, refreshToken, profile, cb)=> {
        try {
            ...
            if(result.rows.length === 0) {
                ...
                const newUser = await db.query(`insert into users(email, auth_provider, google_id, display_name, profile_pic_url)
    ...
    passport.deserializeUser(async (id, cb)=> {
        try {
            ...
            result.rows.length > 0 ? cb(null, result.rows[0]) :cb(new Error("User not found"),null);
        }
        catch(err) {
            cb(err, null);
        }
